...
namespace GameUtil.TileGen
{
    public class TileThemeConfig : ScriptableObject
    {
        ...
        public int GetTilePrefabConfigIndexIgnoreOccurancy(string theme_name) 
        {
            List<TilePrefabConfig> config_list = null;
            if (prefab_index.TryGetValue(theme_name, out config_list) == true && config_list.Count > 0)
            {
        ...
        public int GetTilePrefabConfigIndex(string theme_name)
        {
            List<TilePrefabConfig> config_list = null;
            //存在且不为空
            if (prefab_index.TryGetValue(theme_name, out config_list) == true && config_list.Count > 0)
